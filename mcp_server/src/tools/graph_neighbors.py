...
async def graph_neighbors(entity_name: str, depth: int = 1, include_analyst: bool = True):
    with tracer.start_as_current_span("tool_graph_neighbors"):
        # Enforce safety limits
        if depth > 2:
            depth = 2

        results = repo.get_neighbors(entity_name, depth, include_analyst=include_analyst)
